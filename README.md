...
### 7.3 授权 Permit2 花费 msUSD（上线前必做）

三步撤出中，swap 步骤通过 Permit2 拉取 msUSD。需要先授权 Permit2 合约，再由 Universal Router 通过 Permit2 完成划转。
通过 Basescan 的 Write Contract 功能完成（**用质押钱包签名**）：

1. 进入 msUSD 合约页面（Basescan 搜索 msUSD 合约地址）
2. 点击 **"Write Contract"** → **"approve"**
3. 填写参数：
   - `spender`：Permit2 合约地址（`0x000000000022D473030F116dDEE9F6B43aC78BA3`，Base 链固定地址）
