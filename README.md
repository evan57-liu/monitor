...
execution:
  ...
  swap_pool_param: 0x100001 # Universal Router path 的 3 字节 poolParam：0x100000 = CL Factory 2 标志，0x000001 = tickSpacing=1
...
router_address: "0x6Df1c91424F79E40E33B1A48F0687B666bE71075"        # Aerodrome Universal Router
...
### 7.3 授权 Permit2 花费 msUSD/USDC（上线前必做）

swap 步骤通过 Permit2 从质押钱包拉取 msUSD。需要完成**两层授权**，缺任何一层都会导致 swap revert `0xd81b2f2e`（`AllowanceExpired`）。

> ⚠️ **注意**：Aerodrome 使用自己部署的 Permit2 fork，地址是 `0x494bbd8a3302aca833d307d11838f18dbada9c25`，**不是** Uniswap 官方 Permit2（`0x000000000022D473030F116dDEE9F6B43aC78BA3`）。两者不可互换。

**推荐方式：运行 setup 脚本（自动幂等，可重复执行）**

```bash
# 仅预览需要发哪些交易（不发送）
npx tsx scripts/setup-permit2.ts

# 实际发送交易
npx tsx scripts/setup-permit2.ts --execute

# 验证授权后模拟一次 swap
npx tsx scripts/setup-permit2.ts --simulate-swap
```

**备用方式：Basescan 手动操作（逐步）**

**第一层：ERC20 → Permit2**（让 Permit2 有权转移你的代币）

对 msUSD 和 USDC 各做一次：

1. Basescan 搜索代币合约地址，进入 **"Write Contract"** → **"approve"**
2. `spender`：**Aerodrome Permit2** `0x494bbd8a3302aca833d307d11838f18dbada9c25`
3. `amount`：`115792089237316195423570985008687907853269984665640564039457584007913129639935`（`type(uint256).max`）
4. 用质押钱包连接 MetaMask 签名发送

**第二层：Permit2 → Universal Router**（让 Router 有权通过 Permit2 调用 transferFrom）

对 msUSD 和 USDC 各做一次（在 Permit2 合约页面操作）：

1. 打开 Aerodrome Permit2 合约：[0x494bbd8a...](https://basescan.org/address/0x494bbd8a3302aca833d307d11838f18dbada9c25#writeContract)
2. 点击 **"Write Contract"** → **"approve"**
3. 填写参数：
   - `token`：代币地址（msUSD 或 USDC）
   - `spender`：Universal Router `0x6Df1c91424F79E40E33B1A48F0687B666bE71075`
   - `amount`：`1461501637330902918203684832716283019655932542975`（`type(uint160).max`）
   - `expiration`：`281474976710655`（`type(uint48).max`，永不过期）
4. 用质押钱包签名发送

验证方式：在 Aerodrome Permit2 合约页面 **"Read Contract"** → **"allowance"**，输入
`(owner=质押钱包, token=msUSD, spender=Universal Router)`，应返回 `amount=max, expiration=281474976710655`。
...
# 3. swap 交易（3笔）：Universal Router execute(V3_SWAP_EXACT_IN) 成功，msUSD → USDC
```
