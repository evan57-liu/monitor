...
export class Engine {
  ...
  private makeDataSourceFailureAlert(monitorId: string, err: unknown): Alert {
    return {
      ...
      protocol: monitorId,
      title: `监控器异常：${monitorId}`,
      message: `监控器 \`${monitorId}\` 已连续失败 ${MAX_CONSECUTIVE_FAILURES} 次，已自动暂停 5 分钟。\n\n**最后一次错误：** ${err instanceof Error ? err.message : String(err)}`,
  ...
  private makeExecutionFailureAlert(order: ExecutionOrder, error?: string): Alert {
    return {
      ...
      title: `执行失败：${order.type}（组 ${order.groupId}）`,
      message: `订单 \`${order.id}\`（${order.type}，序号 ${order.sequence}）执行失败，该组剩余订单已中止。\n\n**错误：** ${error ?? '未知'}`,
